...
function encrypt_(data, key, iv="", mode = "ECB", padding = "Pkcs7") {
    const _mode = {
        ...
        "OPENPGP": CryptoJS.mode.OPENPGP,
        "CCM": CryptoJS.mode.CCM,
        "EAX": CryptoJS.mode.EAX,
        "SIV": CryptoJS.mode.SIV,
        "GCM": CryptoJS.mode.GCM,
        "OCB": CryptoJS.mode.OCB
    };
    const _padding = {
        'Pkcs7': CryptoJS.pad.Pkcs7
    };
    const new_iv = CryptoJS.enc.Base64.parse(iv);
    const new_key = CryptoJS.enc.Utf8.parse(key);
    const result = CryptoJS.TripleDES.encrypt(data, new_key, {
        iv: new_iv,
        mode: _mode[mode],
...
var n = (new Date).getTime() - 7565,
    t = "s54zv9bm1vd5czfujy6nnuxj1l4g2ny6",
    a = Y();
    c = S(n + a + S(a + t + n));


function get_parameter(email_, password_) {
    key_= Q(email_);
    password_encrypt = encrypt_(password_, key_);
    result_ = {
        "email": email_,
