var tools = require("./window.js");
var gct_tool = require("./demo_gct.js");
window = tools.window;
document = tools.document();
navigator = tools.navigator();
screen = tools.screen();
location = tools.location();
gct_n = gct_tool.gct_n;
...
var $_CJDQ = lTloj.$_CX, $_CJCo = ['$_CJGh'].concat($_CJDQ), $_CJET = $_CJCo[1];$_CJCo.shift();var $_CJFU = $_CJCo[0];
function $_BHR(t) {
...
// 滑块按钮图DOMRect对象
o_DOMRect = {
    bottom: 534.4609375, // 返回 的底部坐标值
    height: 65.96875, // 对象高度
    left: 758.5,    // 返回 的左坐标值
    right: 824.46875, // 返回 的右坐标值
    top: 468.4921875,// 返回 的顶部坐标值
    width: 65.96875, // 对象宽度
    x: 758.5,   // DOMRect的原点的 x 坐标（通常是矩形的左上角）
    y: 468.4921875, // DOMRect的原点的 y 坐标（通常是矩形的左上角）
};
// 滑块背景图DOMRect对象
s_DOMRect = {
    bottom: 465.5234375,
...
// function init_ae(data_info) {
//     c = ae.$_CFDP;
//     _ = o_DOMRect.left;
//     a = o_DOMRect.top;
//     ae.$_CIBw = new W([Math.round(_ / c - ft.$_DBI.clientX), Math.round(a / c -ft.$_DBI.clientY), 0]).$_BBBF([0, 0 ,0]);
//     ae.$_CIBw.$_HBA = data_info['mouse_data']; // 滑块轨迹
//     ae.$_BHEf = ae.prototype.$_CEHv;
//
//     // 把data_info的元素更新到re.prototype
//     for (var key in data_info) {
//         re.prototype.key = data_info.key
//     }
//     ae.$_CIY = re.prototype
// }

function get_verify_slide_code_w(data_info){

    ae.$_CHJg = data_info['startclickX']; // 初始点击滑块按钮停的x轴坐标
    ae.$_CIAc = data_info['startclickY']; // 初始点击滑块按钮停的y轴坐标
    ae.$_CHIF = data_info['startclicktime']; // 初始点击验证码的时间
    ft.$_DBI = {
        clientX: data_info['endclickX'], // 最后滑块按钮停的x轴坐标
        clientY: data_info['endclickY'], // 最后滑块按钮停的y轴坐标
    };
    init_c = ae.$_CFDP;
    init__ = o_DOMRect.left;
    init_a = o_DOMRect.top;
    ae.$_CIBw = new W([Math.round(init__ / init_c - ft.$_DBI.clientX), Math.round(init_a / init_c -ft.$_DBI.clientY), 0]).$_BBBF([0, 0 ,0]);
    ae.$_CIBw.$_HBA = data_info['mouse_data']; // 滑块轨迹  ae.$_CIBw.$_GEy()方法会调用到ae.$_CIBw.$_HBA这个参数进行加密
    ae.$_BHEf = ae.prototype.$_CEHv;
    ...
    a = ae.$_CFDP;
    _ = ft.$_DBI.clientX / a - ae.$_CHJg;
    c = ae.$_CIAc - ft.$_DBI.clientY / a;
    // ae.$_CECK = $_HP() - data_info['startclicktime'];
    ae.$_CECK = data_info['mouse_data'].slice(-1,)[0][2] + 32;
    ae.$_CIBw.$_BBBF([Math.round(_), Math.round(c), ae.$_CECK]);
    u_CGHV = parseInt(_);

    console.log(ae.$_CIBw.$_GEy());
    l_CGHV = ae.$_CIBw.$_BBCA(ae.$_CIBw.$_GEy(), ae.$_CIY.c, ae.$_CIY.s);

    aeskey = ee.prototype.$_CBFG(); // 随机的aeskey
    console.log(aeskey);
	u = new X().encrypt(aeskey);
	passtime = data_info['mouse_data'].slice(-1,)[0][2]; // 验证码轨迹的时间
	rp = U(data_info['gt'] + data_info['challenge'].slice(0,32) + passtime);
	lp_time = data_info['startclicktime'];
	o = {
	    "lang": re.prototype.lang,
        "userresponse": H(u_CGHV, re.prototype.challenge),
        "passtime": passtime,
        "imgload": Math.round(Math.random()*(20-10)+60),
        "aa": l_CGHV,
        "ep": {
            "v": re.prototype.version,
            "$_BHR": false,
            "me": true,
            // tm自己构造的   PerformanceTiming对象
            "tm": {"a":lp_time-543,"b":lp_time-517,"c":lp_time-517,"d":0,"e":0,"f":lp_time-644,"g":lp_time-644,
                "h":lp_time-544,"i":lp_time-541,"j":lp_time-520,"k":lp_time-520,"l":lp_time-518,"m":lp_time-514,"n":lp_time-513,
                "o":lp_time-512,"p":lp_time-111,"q":lp_time-111,"r":lp_time-108,"s":lp_time-105,"t":lp_time-105,"u":lp_time-105},
            "td": -1
        },
        "rp": rp,
    };
	o["xb3y"] = '1503756539'; // 目前不会被检测
	// gct_n_t = {"lang": o.lang, "ep": o.ep};
	// gct_n(gct_n_t);
	// o["cigl"] = gct_n_t["cigl"];
	// console.log(o);
	l = V.encrypt(gt.stringify(o), aeskey);
    h = m.$_GFm(l);
    w = h + u;
    return {'w': w, 'o': o}
}
...
data_ = {"gt": "019924a82c70bb123aae90d483087f94", "challenge": "55b827bc3fe8f00909ef7488d572fda5kf", "id": "a55b827bc3fe8f00909ef7488d572fda5",
    "bg": "pictures/gt/a3e9020a7/bg/247dd1d88.jpg", "fullbg": "pictures/gt/a3e9020a7/a3e9020a7.jpg",
    "link": "", "ypos": 35, "xpos": 0, "height": 160, "slice": "pictures/gt/a3e9020a7/slice/247dd1d88.png",
    "api_server": "https://api.geetest.com", "static_servers": ["static.geetest.com/", "dn-staticdown.qbox.me/"],
    "mobile": true, "theme": "ant", "theme_version": "1.2.6", "template": "", "logo": true, "clean": false,
    "type": "multilink", "fullpage": false, "feedback": "https://www.geetest.com/contact#report", "show_delay": 250,
    "hide_delay": 800, "benchmark": false, "version": "6.0.9", "product": "popup", "https": true, "width": "100%",
    "show_voice": true, "c": [12, 58, 98, 36, 43, 95, 62, 15, 12], "s": "7370325a", "so": 0,
    "i18n_labels": {"cancel": "取消", "close": "关闭验证", "error": "请重试", "fail": "请正确拼合图像",
        "feedback": "帮助反馈", "forbidden": "怪物吃了拼图，请重试", "loading": "加载中...",
        "logo": "由极验提供技术支持", "read_reversed": false, "refresh": "刷新验证", "slide": "拖动滑块完成拼图",
        "success": "sec 秒的速度超过 score% 的用户", "tip": "请完成下方验证", "voice": "视觉障碍"},
    "gct_path": "/static/js/gct.c4cd672f9bda085650bb6b6caa9e68e6.js", "startclickX": 789, "startclickY": 489,
    "startclicktime": 1640679488363, "endclickX": 889, "endclickY": 499,
    "mouse_data": [[-31, -21, 0], [0, 0, 0], [0, 0, 21], [0, 0, 39], [1, 0, 66], [2, 0, 91], [3, 1, 103], [4, 1, 117], [5, 1, 138], [8, 2, 166], [10, 2, 185], [12, 3, 200], [15, 3, 220], [17, 4, 236], [21, 4, 255], [25, 5, 279], [27, 5, 291], [31, 6, 310], [37, 7, 334], [43, 7, 359], [48, 8, 379], [55, 9, 404], [60, 9, 426], [64, 9, 440], [67, 9, 455], [73, 10, 482], [76, 10, 501], [82, 10, 529], [84, 10, 547], [86, 10, 559], [89, 10, 581], [91, 10, 593], [93, 10, 611], [95, 10, 629], [97, 10, 656], [99, 10, 681], [100, 10, 702], [100, 10, 715], [100, 10, 736], [100, 10, 761], [100, 10, 775], [100, 10, 800]]};

result = get_verify_slide_code_w(data_);
console.log(result);